...
export function SideBar({ plot, plots, onChange, onChangeTransient, onDuplicate, onRemove }: Props) {
	if (!plot) {
	...
	return (
		<SideBarContent
			...
			plots={plots}
			onChange={onChange}
			onChangeTransient={onChangeTransient}
...
function SideBarContent({ plot, plots, onChange, onChangeTransient, onDuplicate, onRemove }: ContentProps) {
	const selection = useMemo(() => new Set<PointId>(plot.selection), [plot.selection]);
	const selectedPoints = useMemo(() => plot.points.filter(p => selection.has(p.id)), [plot.points, selection]);
